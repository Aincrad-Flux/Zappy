...
static int find_team_by_name(Server *server, const char *team_name)
{
    for (int i = 0; i < server->num_teams; i++) {
...
static int can_join_team(Server *server, int team_id)
{
    if (team_id == -1)
        return 0;
...
void handle_team_join_success(Server *server, int client_socket, int team_id,
    const char *team_name)
{
    ...
    init_player(&server->players[server->num_players],
            client_socket, team_id, team_name, server);
...
void handle_team_authentication(Server *server, int client_socket,
    const char *team_name)
{
