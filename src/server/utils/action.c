...
void add_action_to_queue(Player *player, const char *command, int freq) {
    ...
    if (player->action_queue == NULL) {
        new_action->end_time = base_time + (int)ceil((double)duration_ticks / freq);
        new_action->next = NULL;
        player->action_queue = new_action;
    } else {
        Action *curr = player->action_queue;
        while (curr->next)
            curr = curr->next;
        new_action->end_time = curr->end_time + (int)ceil((double)duration_ticks / freq);
        new_action->next = NULL;
...
void handle_info_command(Player *player, Server *server, const char *command, char *response) {
    if (strcmp(command, "Look") == 0) {
        ...
        snprintf(response, BUFFER_SIZE, "%d\n",
                 server->teams[player->team_id].max_clients - server->teams[player->team_id].current_clients);
    }
}

void handle_movement_command(Player *player, Server *server, const char *command, char *response) {
...
void process_player_command(Player *player, Server *server, const char *command) {
    char response[BUFFER_SIZE] = "";

    handle_movement_command(player, server, command, response);
    if (response[0] == '\0') handle_info_command(player, server, command, response);
    if (response[0] == '\0') handle_action_command(command, response);
...
static void handle_action(Server *server, int i)
{
    ...
    Action *current_action = player->action_queue;
